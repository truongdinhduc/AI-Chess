...
ROOK_PHASE = 2
QUEEN_PHASE = 4
TOTAL_PHASE = PAWN_PHASE*16 + KNIGHT_PHASE*4 + \
    ...
    for square in range(64):
        ...
        if piece is not None:
            if piece.color == chess.WHITE:
                # piece.piece_type : name of chess piece
                mg[piece.color] += MG_PESTO[piece.piece_type][63-square] + \
                    MG_PIECE_VALUES[piece.piece_type]
                eg[piece.color] += EG_PESTO[piece.piece_type][63-square] + \
                    EG_PIECE_VALUES[piece.piece_type]
            else:
                mg[piece.color] += MG_PESTO[piece.piece_type][square] + \
                    MG_PIECE_VALUES[piece.piece_type]
                eg[piece.color] += EG_PESTO[piece.piece_type][square] + \
                    EG_PIECE_VALUES[piece.piece_type]
...
def evaluate_piece(board: chess.Board, square: chess.Square, phase: float) -> float:
    ...
    if piece is not None:
        if piece.color == chess.WHITE:
            mg_score += MG_PESTO[piece.piece_type][63 -
                                                   square] + MG_PIECE_VALUES[piece.piece_type]
            eg_score += EG_PESTO[piece.piece_type][63 -
                                                   square] + EG_PIECE_VALUES[piece.piece_type]
        else:
            mg_score += MG_PESTO[piece.piece_type][square] + \
                MG_PIECE_VALUES[piece.piece_type]
            eg_score += EG_PESTO[piece.piece_type][square] + \
                EG_PIECE_VALUES[piece.piece_type]
...
def evaluate_capture(board: chess.Board, move: chess.Move, phase: float) -> float:
    ...
    if capturing_piece is not None and captured_piece is not None:
        mg_score += MG_PIECE_VALUES[captured_piece] - \
            MG_PIECE_VALUES[capturing_piece]
        eg_score += EG_PIECE_VALUES[captured_piece] - \
            EG_PIECE_VALUES[capturing_piece]
