...
from PeSTOS import evaluate_capture, evaluate_piece, get_phase
...
def organize_moves_quiescence(board: Board):
    ...
    captures = filter(lambda move: board.is_zeroing(
        move) or board.gives_check(move), board.legal_moves)
    # sort moves by importance
    moves = sorted(captures, key=lambda move: mvv_lva(
        board, move, phase), reverse=(True if board.turn == BLACK else False))
    return moves
